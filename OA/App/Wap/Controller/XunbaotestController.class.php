...
use Think\Model;
class XunbaotestController extends Controller {	
public $wxuser = array();	
public function _initialize(){
$weixinid=2;
if(I('get.from')){
redirect(U('Xunbaotest/'.ACTION_NAME.'?fx_id='.I('get.fx_id').'&hdid='.I('get.hdid')));
	}
	...
	if(empty($id))
	{
		...
		if(!$udata||!( $udata['id']==session('uid')&&$udata['wx']==session('wx'))||$udata['partner_id']!=$weixinid||1){
			if($code){
 ...
 $userinfo = json_decode($userinfo, true);
		...
		$this->wxuser=$wx_user =M(tablecopy('wxuser',$weixinid))->getByWx($openid); 
					...
					redirect(U('xunbaotest/index?fx_id='.$fx_id));
				}
			}else
			{
				header("location: https://open.weixin.qq.com/connect/oauth2/authorize?appid=".$weixin['appid']."&redirect_uri=".'http://www.dscm.com.cn'.urlencode('/xcrm/index.php?s=/Wap/xunbaotest/index/fx_id/'.$fx_id)."&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect");
			}
		}else{	
			$this->wxuser=$udata;
		}
...
public function index()
{
	...
	$zlrs = round($zhulirs['total']/20); //向上向下取整；
	...
	if($fx_id && $id) 
	{
		$fx_user = M(tablecopy('wxuser',$weixinid))->getById($fx_id);
		//获取分享者的助力信息；
