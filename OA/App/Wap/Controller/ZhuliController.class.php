...
if(I('get.from')){
redirect(U('Zhuli/'.ACTION_NAME.'?fx_id='.I('get.fx_id').'&hdid='.I('get.hdid')));
	}
		...
		if(!$udata||!( $udata['id']==session('uid')&&$udata['wx']==session('wx'))||$udata['partner_id']!=$weixinid){
			if($code){
				...
				if($openid)
				{
					...
					redirect(U('Zhuli/index?fx_id='.$fx_id.'&hdid='.$hdid));
				}
			}else
			{
				header("location: https://open.weixin.qq.com/connect/oauth2/authorize?appid=".$weixin['appid']."&redirect_uri=".'http://www.dscm.com.cn'.urlencode('/xcrm/index.php?s=/Wap/Zhuli/index/fx_id/'.$fx_id.'/hdid/'.$hdid)."&response_type=code&scope=snsapi_base&state=STATE#wechat_redirect");
			}
		}else{	
			$this->wxuser=$udata;
		}
...
public function show()
{
	...
	$this->display(T('Zhuli/show'.$hdid));
}
...
/*	$style['top']=I('post.image');
	...
	//排行榜TOP名；
	$Model = new Model();
	$zhulirs = $Model->query("select * from __PREFIX__zhuli where hdid = '$hdid' and uid != 0  GROUP BY uid order by zlb desc limit 150");
	...
	$zlrs = round(($zhulirs[0]['total']-80*$cjrs)/20); //向上向下取整；
	//print_r($zhulirs);
	...
	//echo T('Zhuli/index'.$hdid);
	$this->display(T('Zhuli/index'));
}
			...
			if($j){ 
				...
				if($hdid==59 || $hdid ==60 || $hdid==58) 
				{
					if(mb_substr($json_data["data"]['region'],0,2, 'utf-8') == "山东" )
					{
						...
						echo '成功为您好友 增加20助力币,'.($wx_zl['zlb']+20);
					}
					else
					{
						echo '您已经助力过了,'.$wx_zl['zlb'];
