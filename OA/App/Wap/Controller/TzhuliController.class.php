...
use Think\Model;
class TzhuliController extends Controller {	
public $wxuser = array();
public function _initialize(){
if(I('get.from')){
redirect(U('Tzhuli/'.ACTION_NAME.'?fx_id='.I('get.fx_id').'&hdid='.I('get.hdid')));
	}
		...
		if(!$udata||!( $udata['id']==session('uid')&&$udata['wx']==session('wx'))||$udata['partner_id']!=$weixinid){
			if($code){
				...
				if($openid)
				{
					...
					redirect(U('Tzhuli/index?fx_id='.$fx_id.'&hdid='.$hdid));
				}
			}else
			{
				header("location: https://open.weixin.qq.com/connect/oauth2/authorize?appid=".$weixin['appid']."&redirect_uri=".'http://www.dscm.com.cn'.urlencode('/xcrm/index.php?s=/Wap/Tzhuli/index/fx_id/'.$fx_id.'/hdid/'.$hdid)."&response_type=code&scope=snsapi_base&state=STATE#wechat_redirect");
			}
		}else{	
			$this->wxuser=$udata;
		}
...
public function show()
{
	...
	$this->display(T('Tzhuli/show'.$hdid));
}
...
public function index()
{
	...
	//echo T('Tzhuli/index'.$hdid);
	$this->display(T('Tzhuli/index'.$hdid));
}
...
public function weixinadd(){
	...
	{			
		...
		redirect(U('Tzhuli/index?hdid='.$hdid)); 
	}else{
		//更新用户信息，填写用户姓名和手机号；
		/*$Model = new Model();
...
//设置刷票拦截机制
$black = array(234555,245438,240415,391724);
			...
			if($j){
				...
				if(mb_substr($json_data["data"]["city"],0,2, 'utf-8') == "枣庄" )
				{
					...
					if($hdid == 53)
					{
						...
						echo '成功助力为您好友 增加1助力币,'.($wx_zl['zlb']+1);
					}
					else
					{
...
//设置刷票拦截机制
$black = array(242391,238556,245438,240415,257861,245939,375867,245408,224733);
