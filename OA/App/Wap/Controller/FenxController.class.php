		...
		if(!$udata||!( $udata['id']==session('uid')&&$udata['wx']==session('wx'))||$udata['partner_id']!=$weixinid){
			if($code){
				...
				if($openid)
				{
					...
					redirect(U('Fenx/index?fx_id='.$fx_id.'&hdid='.$hdid));
				}
			}else
			{
				header("location: https://open.weixin.qq.com/connect/oauth2/authorize?appid=".$weixin['appid']."&redirect_uri=".'http://www.dscm.com.cn'.urlencode('/xcrm/index.php?s=/Wap/Fenx/index/fx_id/'.$fx_id.'/hdid/'.$hdid)."&response_type=code&scope=snsapi_base&state=STATE#wechat_redirect");
			}
		}else{	
			$this->wxuser=$udata;
		}
...
public function index()
{
	...
	//活动参加人数；
	$Fenx = M('Fenxiang');
	$t_join = $Fenx->where("hdid='$hdid'")->count();
	$this->assign('t_join',$t_join);
	//活动分享次数；
	$Fenx = M('Fenxiang');
	$t_fxcs = $Fenx->where("hdid='$hdid'")->sum("number");
	$this->assign('t_fxcs',$t_fxcs);
	//活动总分享币；
	$Fenx = M('Fenxiang');
	$t_fxb = $Fenx->where("hdid='$hdid'")->sum("fxb");
	...
	{
		$Fenx = M('Fenxiang');
		$wx_fx = $Fenx->where("hdid = '$hdid' and uid = '$id'")->order('fxb desc')->find();
		//print_r($wx_zl);
	...
	if($fx_id && $id)
	{
		$Fenx = M("Fenxiang"); // 实例化User对象
		$map['uid'] = $fx_id;
		$map['hdid'] = $hdid;// 把查询条件传入查询方法
		$wx_fx = $Fenx->where($map)->order('fxb desc')->find();
		$user_list = explode(',',$wx_fx['fxjl']);
		//判断活动是否已经结束；
		if($news['jssj']>time()){
			
			//如果分享用户在黑名单中，直接返回；
			if(in_array($id,$black))
			{
				$is_help = 1; // 已经帮其增加阅读数；	
			}
			else
			{
				//用户不在列表时，更新操作，增加助力值；
				if(!in_array($id,$user_list) && $wx_fx)
				{                                     
					$ip = getClientIp();
	...
	//echo T('Fenx/index'.$hdid);
	$this->display(T('Fenx/index'));
}
...
public function weixinadd(){
	...
	if(empty($wx_has_in))
	{
		...
		redirect(U('Fenx/index?hdid='.$hdid)); 
	}else{
		//更新用户信息，填写用户姓名和手机号；
		/*$Model = new Model();
...
public function fenxiangapi(){
	...
	//分享信息；
	$Fenx=M('Fenxiang');
	$fx_res=$Fenx->where('hdid='.$hdid.' and uid='.$wx_id)->order('fxb desc')->find();
	//判断活动是否已经结束；
	if($news['jssj']>time()){
		//如果用户参加活动；
		if($fx_res && $fx_res['name'] && $fx_res['tel'])
		{
			...
			if($wx_id && $fx_id == $wx_id)
			{
				...
				if($time['yday'] !=  $time_n['yday'])
				{
					...
					$Fenx->where("id=".$fx_res['id'])->save($data);
				}
			}
			// 其他用户分享的内容；
			else
