...
public function _initialize(){
	...
	if(session('uid')){
		$this->wxuser=$udata=M('wxuser')->getById(session('uid'));
	}
	if(!$udata||!( $udata['id']==session('uid')&&$udata['wx']==session('wx'))||$udata['partner_id']!=$weixinid){
		if($code){
			...
			if($openid)
			{
				...
				redirect(U('Check/index?fx_id='.$fx_id));
			}
		}else
		{
			header("location: https://open.weixin.qq.com/connect/oauth2/authorize?appid=".$weixin['appid']."&redirect_uri=".'http://www.dscm.com.cn'.urlencode('/xcrm/index.php?s=/Wap/Check/index/fx_id/'.$fx_id)."&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect");
		}
...
public function index()
{
	...
	$Model= M('Zhuli');
	$wxuser=$this->wxuser;
	$fx_user = M('wxuser')->getById(I('fx_id'));
	...
	if($cgzls>0){
		...
		if(IS_POST){
			...
			if(M('wxuser')->save($fx_user)&&$Model->save($wx_zl)){
				$data['mid']=$wxuser['id'];
				$data['uid']=$fx_user['id'];
				$data['qx']=$wxuser['qx'];
				$data['time']=time();
