...
class AdminService extends CommonService {
    ...
    return $str;
	 ...
	 public function getAccessToken($pid,$weixin) {
   ...
   public function getJsApiTicket($pid,$weixin) {
    ...
    if ($data['expire_time'] < time()) {
     $accessToken = $this->getAccessToken($pid,$weixin);
 ...
 public function getSignPackage($pid,$weixin) {
	 	if(!$weixin){
	$weixin =M('weixin')->getById($pid); 
	}
 $jsapiTicket = $this->getJsApiTicket($pid,$weixin);
