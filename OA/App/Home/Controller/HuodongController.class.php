...
class HuodongController extends CommonController{
  ...
  public function _befor_add(){
	  $weixin=M('weixin');
	  $weixinlist=$weixin->getField('id,weixin');
	  $attid=time();  
	  $this->assign('weixinlist',$weixinlist);
...
$prize_arr = $prize_arr = array( '0' => array('id'=>1,'min'=>5,'max'=>25,'prize'=>'一等奖','v'=>0,'sl'=>'0'), 
    ...
    '4' => array('id'=>5,'min'=>125,'max'=>145,'prize'=>'五等奖','v'=>0,'sl'=>0), 
    '5' => array('id'=>6,'min'=>65,'max'=>85,'prize'=>'六等奖','v'=>0,'sl'=>0), 
    '6' => array('id'=>7,'min'=>array(35,95,155,215,275,335),'max'=>array(55,115,175,235,295,355),'prize'=>'没中奖','v'=>100000,'sl'=>'100000') ); 
$jpname=I('post.jpname');
$jpgl=I('post.jpgl');
$jpsl=I('post.jpsl');
foreach($prize_arr as $key=> $c){
...
$data['jp']=serialize($prize_arr);
    ...
    public function _befor_addstyle($data){
...
$data['style']=serialize($style);
 return $data;
    ...
    public function _after_edit($data){
	  ...
	  public function _format_styleedit($vo){
	  $vo['style']=unserialize($vo['style']) ;
	  return $vo;
	  }	 
